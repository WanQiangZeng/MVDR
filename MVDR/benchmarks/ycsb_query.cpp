...
void ycsb_query::init(uint64_t thd_id, workload *h_wl, Query_thd *query_thd) {
  ...
  requests = (ycsb_request *)mem_allocator.alloc(
      sizeof(ycsb_request) * local_req_per_query, thd_id);
  part_to_access = (uint64_t *)mem_allocator.alloc(
      sizeof(uint64_t) * g_part_per_txn, thd_id);
...
// 以下算法来自YCSB的paper: Quickly generating billion-record synthetic
// databases
...
// The return value range is [1, n]
uint64_t ycsb_query::zipf(uint64_t n, double theta) { // theta越大倾斜越明显
#if !SYNTHETIC_YCSB
  assert(this->the_n == n);
#endif
...
void ycsb_query::gen_requests(uint64_t thd_id, workload *h_wl) {
  ...
  for (tmp = 0; tmp < local_req_per_query; tmp++) {
    ...
    UInt32 hs_idx =
        (UInt32)min((int)g_req_per_query - 1,
                    max(1, (int)floor(g_req_per_query * g_specified_ratio)));
    if (tmp == hs_idx) {
      req->rtype = FIRST_HS;
      row_id = table_size - 1;
    } else {
...
//*****************************************
    ...
    UInt32 hs1_idx = 0;
#else
    UInt32 hs2_idx = (UInt32)min(
        (int)g_req_per_query - 2,
        max(0, (int)floor(g_req_per_query * (1 - g_specified_ratio))));
...
//**************************************
    ...
    if (tmp == (g_req_per_query - 1)) {
      ...
      // uint64_t table_size = g_synth_table_size / g_virtual_part_cnt;
      // uint64_t row_id = zipf(table_size - 1, g_zipf_theta);
      row_id = get_new_row(); // 根据倾斜程度生成此req的row_id
#if SYNTHETIC_YCSB
    }
#endif
