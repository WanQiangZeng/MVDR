...
RC tpcc_wl::init_schema(const char *schema_file) {
  ...
  tables["WAREHOUSE"]->set_table_name("WAREHOUSE");
  tables["WAREHOUSE"]->schema->table_name = "WAREHOUSE";
  t_warehouse = tables["WAREHOUSE"];

  tables["DISTRICT"]->set_table_name("DISTRICT");
  tables["DISTRICT"]->schema->table_name = "DISTRICT";
  t_district = tables["DISTRICT"];

  tables["CUSTOMER"]->set_table_name("CUSTOMER");
  tables["CUSTOMER"]->schema->table_name = "CUSTOMER";
  t_customer = tables["CUSTOMER"];

  tables["HISTORY"]->set_table_name("HISTORY");
  tables["HISTORY"]->schema->table_name = "HISTORY";
  t_history = tables["HISTORY"];

  tables["NEW-ORDER"]->set_table_name("NEW-ORDER");
  tables["NEW-ORDER"]->schema->table_name = "NEW-ORDER";
  t_neworder = tables["NEW-ORDER"];

  tables["ORDER"]->set_table_name("ORDER");
  tables["ORDER"]->schema->table_name = "ORDER";
  t_order = tables["ORDER"];

  tables["ORDER-LINE"]->set_table_name("ORDER-LINE");
  tables["ORDER-LINE"]->schema->table_name = "ORDER-LINE";
  t_orderline = tables["ORDER-LINE"];

  tables["ITEM"]->set_table_name("ITEM");
  tables["ITEM"]->schema->table_name = "ITEM";
  t_item = tables["ITEM"];

  tables["STOCK"]->set_table_name("STOCK");
  tables["STOCK"]->schema->table_name = "STOCK";
