...
void IndexHash::get_latch(BucketHeader *bucket, access_t access) {
  while (!ATOM_CAS(bucket->locked, false, true)) {
  }
  if (access == RD)
...
void BucketHeader::read_item(idx_key_t key, itemid_t *&item,
  ...
  BucketNode *cur_node = first_node;
  if (cur_node == NULL) {
    fprintf(stderr, "Error: first_node is NULL in read_item\n");
    return;
  }
  while (cur_node != NULL) {
    if (cur_node->key == key)
  ...
  if (cur_node == NULL) {
    fprintf(stderr, "Error: Key %lu does not exist in read_item\n", key);
    return;
  }
