    ...
    if (request->rtype == RD || request->rtype == WR) {
      { // 对整个版本链加锁
        ...
        if (request->rtype == WR) {
          ...
          if (write_it == NULL ||
            ...
            // 需要修复WW冲突
              ...
              for (; write_it != NULL; write_it = write_it->next) {
                ...
                // repair WW conflict
                repair_deque(write_it->data.mv_id.qid, 0);
          ...
          // else if (write_it->data.mv_id.qid == mv_newnode.mv_id.qid) {
          //   write_it = write_it->next;
          //   for (; write_it != NULL; write_it = write_it->next) {

          //     // repair WW conflict,从下一个开始
          //     // usleep(50);
          //     repair(write_it->data.mv_id.qid, write_it->data.mv_id.rid + 1);
          //     ATOM_ADD(repair_cnt, 1);
          ...
          row_tmp = static_cast<row_t *>(item_tmp->location);
          {
            ...
            if (it_tmp != NULL) {
              ...
              for (; it_tmp != NULL; it_tmp = it_tmp->next) {
                ...
                // repair WW conflict
                repair_deque(it_tmp->data.mv_id.qid, 0);
          ...
          if (query->be_dep_cnt > g_be_dep_cnt) {
            for (auto it = req_deque.begin(); it != req_deque.end(); it++) {
              ...
              if (query_tmp->be_dep_cnt < query->be_dep_cnt) {
                req_deque.insert(it, make_pair(id.first, query->next_rid));
                break;
              }
            }
          } else {
            req_deque.push_back(make_pair(id.first, query->next_rid));
          }
        }
        query->next_rid++;
        // query->rerun = false;
        ...
        if (query->next_rid == query->request_cnt) {
        ...
        } else if (query->next_rid < query->request_cnt) {
          {
            ...
            if (query->be_dep_cnt > g_be_dep_cnt) {
              ...
              // req_deque.push_front(make_pair(id.first, query->next_rid));
            } else {
              req_deque.push_back(make_pair(id.first, query->next_rid));
            }
