...
// 已实现2+3的一部分
#if WORKLOAD == YCSB
void *run_ycsb_mvdr_no_prio_interactive(void *args) {
  ...
  while (finished_cnt < query_cnt * g_client_cnt) {
    ...
    bool queue_empty;
    {
      ...
      queue_empty = to_commit_query.empty();
      if (!queue_empty) {
        query_id = to_commit_query.front();
        to_commit_query.pop();
      }
    }
    if (!queue_empty) {
            ...
            if (it_tmp->next != NULL) {
                       ...
                       ->queries[next_query_id % query_cnt_per_thd];
              {
                ...
                if (next_query->dep_cnt == 0 && next_query->able_to_commit &&
                  ...
                  {
                    lock_guard<mutex> lock(to_commit_query_mutex);
                    to_commit_query.push(next_query_id);
                  }
                }
              }
            }
    ...
    { // 对整个版本链加锁
      ...
      if (request->rtype == WR) {
        ...
        memcpy(mv_newnode.data, last_mvnode->data, data_size);
        write_mv_node(row, &mv_newnode);
        ...
        if (write_it == NULL ||
          ...
          // 需要修复WW冲突
            ...
            for (; write_it != NULL; write_it = write_it->next) {
              ...
              // repair WW conflict
              repair(write_it->data.mv_id.qid, write_it->data.mv_id.rid);
          ...
          get_itemid(wl, request_tmp->key, item_tmp);
          {
            ...
            if (it_tmp != NULL) {
              ...
              for (; it_tmp != NULL; it_tmp = it_tmp->next) {
                ...
                // repair WW conflict
                repair(it_tmp->data.mv_id.qid, it_tmp->data.mv_id.rid);
