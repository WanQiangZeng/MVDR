...
void itemid_t::init_mvlist() {
  mv_data initial_node;
  initial_node.mv_id = {-1, -1, -1};

  row_t *row = (row_t *)location;
  char *row_data = row->get_data(); // row中的元数据
  ...
  initial_node.commited = true;
  mv_list.push_back(initial_node);
}

// 在workload::index_insert中初始化
void itemid_t::init() {
  valid = false;
  location = 0;
  next = NULL;
  waiter = set<pair<uint64_t, access_t>>();
}
