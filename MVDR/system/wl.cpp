...
RC workload::init_schema(string schema_file) {
  ...
  while (getline(fin, line)) {
    if (line.compare(0, 6, "TABLE=") == 0) {
      ...
      const char *const_str = tname.c_str();
      char *modifiable_str = new char[strlen(const_str) + 1];
      strcpy(modifiable_str, const_str);
      schema->init(modifiable_str, lines.size());
      delete[] modifiable_str;
    ...
    } else if (!line.compare(0, 6, "INDEX=")) { // 索引
      string iname;
      iname = &line[6];
      getline(fin, line);
...
#elif WORKLOAD == TEST
      index->init(part_cnt, tables[tname], BUCKET_CNT);
#endif
#else
...
void workload::index_insert(INDEX *index, uint64_t key, row_t *row,
  ...
  {
    unique_lock<mutex> lock(m_item->mvlist_mutex);
    m_item->init();
    m_item->type = DT_row;
    m_item->location = row;
    m_item->valid = true;
